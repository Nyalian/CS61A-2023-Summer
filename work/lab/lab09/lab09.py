...
def flip_two(s):
    ...
    "*** YOUR CODE HERE ***"
    temp_link = s
    while temp_link is not Link.empty and temp_link.rest is not Link.empty:
        temp = temp_link.first
        temp_link.first = temp_link.rest.first
        temp_link.rest.first = temp
        temp_link = temp_link.rest.rest
...
def multiply_lnks(lst_of_lnks):
    ...
    for link in lst_of_lnks:
        if link is Link.empty:
            return Link.empty
        new_link.first *= link.first
    new_link.rest = multiply_lnks([l.rest for l in lst_of_lnks])
    return new_link
