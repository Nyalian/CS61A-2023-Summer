...
def ordered_digits(x):
    ...
    if x<10:
        return True
    part = lambda x: (x//10, x//10%10, x%10)
    n, prev, next = part(x)
    if prev > next:
        return False
    return ordered_digits(n)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y), 'DivisionError'
    assert lower_bound(y), 'DivisionError'
...
def preorder(t):
    ...
    lists = []
    for b in branches(t):
        lists = lists + preorder(b)
    return [label(t)] + lists
