...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        i = 0
        while not place.is_hive and not (place.bees and self.lower_bound <= i <= self.upper_bound):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if self:
            target.slow_length = self.slow_turns
        else:
            target.slow_length = 0
        def func(gamestate):
            if target.slow_length > 0:
                ...
                Bee.action(target, gamestate)
        if not target.is_slow and not target.is_scared:
            target.action = func
            target.is_slow = True
        # END Problem EC 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        #todo
        if not target.is_slow:
            SlowThrower.throw_at(None, target)
        ...
        def func(gamestate):
            if target.scare_length > 0:
                if target.place.entrance.is_hive:
                    target.scare_length -= 1
                    target.slow_length -= 1
                elif target.slow_length > 0 and gamestate.time % 2 == 1:
        ...
        if target.is_scared == 0:
            target.scare(self.scary_turns)
            target.action = func
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            ...
            if ant and ant is not self:
                dict[ant] = distance
                if ant.is_container and ant.ant_contained:
                    dict[ant.ant_contained] = distance
            place = place.entrance
            distance += 1
        return dict
        # END Problem Optional 2
