...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    lists = [s for s in paragraphs if select(s)]
    return lists[k] if k < len(lists) else ''
    # END PROBLEM 1
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(s):
        return any([x in split(remove_punctuation(lower(s))) for x in subject])
    return func
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not typed_words or not source_words:
        return 0.0
    return sum([typed_words[i] == source_words[i] for i in range(min(len(typed_words), len(source_words)))]) / len(typed_words) * 100.0
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed,source) not in cache:
            ...
            cache[(typed,source)] = {"result":result,"max_limit":limit}
            cache[(source,typed)] = {"result":result,"max_limit":limit}
            return result
        if cache[(typed,source)]["result"] >= 10000 and cache[(typed,source)]["max_limit"] <= limit:
            result = diff_function(typed, source, limit)
            cache[(typed,source)] = {"result":result,"max_limit":limit}
            cache[(source,typed)] = {"result":result,"max_limit":limit}
            return result
        return cache[(typed,source)]["result"]
        # END PROBLEM EC
    return memoized
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff <= limit:
            lists.append((word,diff))
    return min(lists,key=lambda x: x[1])[0] if lists else typed_word
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    index = [min([player_num for player_num in player_indices] , key=lambda player_num: time(match, player_num, word_index)) for word_index in word_indices]
    return [[get_word(match, word_index) for word_index in word_indices if index[word_index] == player_num] for player_num in player_indices]
    # END PROBLEM 10
